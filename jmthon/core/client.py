...
class JmthonClient(TelegramClient):
    ...
    async def start_client(self, **kwargs):
        ...
        try:
            ...
            self.logger.critical(f"❃ فشل الاتصال بخوادم تليجرام: {type(e).__name__}: {e}")
            sys.exit()
        self.me = await self.get_me()
        if self.me is None:
            if kwargs.get("bot_token"):
                try:
                    await self.sign_in(bot_token=kwargs["bot_token"])
                    self.me = await self.get_me()
                except (AccessTokenExpiredError, AccessTokenInvalidError):
                    jmdB.del_key("BOT_TOKEN")
                    self.logger.critical("❃ توكن البوت منتهي او غير صالح")
                    sys.exit()
                except Exception as ex:
                    self.logger.critical(f"❃ {type(ex).__name__}: {ex}")
                    sys.exit()
            else:
