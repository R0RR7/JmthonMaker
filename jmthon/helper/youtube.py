...
from jmthon import LOGS, jmdB 
from jmthon.core.helper import humanbytes
        ...
        for num, file in enumerate(info["entries"]):
            ...
            if file.endswith(".part"):
                ...
                await event.client.send_message(
                    event.chat_id,
                    f"⌔∮ `[{num}/{total}]` `**صيغة الفيديو غير صالحة.\n❃ تم الغاء الطلب**",
                )
                return
    ...
    if type == "video":
        ...
        for vid in data["formats"]:
            ...
            if vid["vcodec"] != "none":
                ...
                if _size < 2147483648:  # 2GB حد التليجرام
                    _video = {}
                    _video.update(
                        {
                            "ytid": id,
...
def get_buttons(listt):
    ...
    buttons.append([Button.inline("رجوع", f"ytdl_back:{id}")])
    return buttons


async def dler(event, url, opts: dict = {}, download=False):
    await event.edit("**⌔∮ جار جلب البيانات**")
    if "quiet" not in opts:
        opts["quiet"] = True
    opts["username"] = jmdB.get_key("YT_USERNAME")
    opts["password"] = jmdB.get_key("YT_PASSWORD")
    if download:
